...
*Alt+Mouse Wheel*::
Adjust the currently selected all-window opacity mode in 5% steps.

*Ctrl+Alt+Mouse Wheel*::
Adjust terminal text opacity in all windows in 5% steps without changing the
terminal background.

*Shift+Alt+Mouse Wheel*::
Move the current window's vertical transparency boundary. The movement step is
configurable and defaults to 1%. Content above the boundary is fully
transparent; content below it is unchanged.
